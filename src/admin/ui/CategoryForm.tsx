...
export function CategoryForm({
  ...
  // A rejected submit echoes what was typed; see FormEcho in actions.ts.
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="mt-1.5 grid gap-3 sm:grid-cols-2">
                <div>
                  ...
                  <input
                    name="nameEn"
                    defaultValue={kept("nameEn", category?.name.en)}
                    className={`mt-1 h-10 ${input}`}
                  />
                  {err.nameEn ? (
                    <p className="text-sale mt-1 text-[11px] font-medium">
                      {err.nameEn}
                    </p>
                  ) : null}
                </div>
                <div>
                  ...
                  <input
                    name="nameAr"
                    defaultValue={kept("nameAr", category?.name.ar)}
                    dir="rtl"
                    className={`mt-1 h-10 ${input}`}
                  />
                  {err.nameAr ? (
            ...
            <div>
              ...
              <div className="mt-1.5 grid gap-3 sm:grid-cols-2">
                <input
                  name="blurbEn"
                  defaultValue={kept("blurbEn", category?.blurb?.en)}
                  className={`h-10 ${input}`}
                />
                <div>
                  <input
                    name="blurbAr"
                    defaultValue={kept("blurbAr", category?.blurb?.ar)}
                    dir="rtl"
                    className={`h-10 ${input}`}
                  />
                  {err.blurbAr ? (
        ...
        <section className="rounded-card bg-white p-6 shadow-[var(--shadow-soft)]">
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <select
                name="department"
                defaultValue={kept("department", category?.department)}
            ...
            <div>
              ...
              <select
                name="art"
                defaultValue={kept("art", category?.art)}
                className={`mt-1.5 h-10 ${input}`}
              >
                <option value="">—</option>
                {arts.map((a) => (
            ...
            <div>
              ...
              <input
                name="slug"
                defaultValue={kept("slug", category?.slug)}
